...
next
  ...
  show ?case
    by (metis (no_types, lifting) bit_word_rotr_iff len_gt_0 mod_add_left_eq
                                  mod_less_divisor test_bit_eq_bit word_rot_gal)
...
next
  ...
  show ?case 
    by (simp add: word_rotr_eq rotater1_rotater_swap word_bl.Abs_inverse
      rev_nth test_bit_bl word_size Suc_diff_Suc)
      (simp add: mod_Suc)
qed
...
lemma eq_sub_equiv_aux:
  "(\<forall>j. uint i \<le> j \<and> j < uint i + uint n \<longrightarrow>
   ...
   (b AND (mask (unat n) << unat i) = a AND (mask (unat n) << unat i))" (is "?P = ?Q")
proof
...
proof -
  ...
  also have "(n - j mod n) mod n = (n mod n - j mod n) mod n"
    by simp
  finally show ?thesis by (simp add: pull_mods(1,5) [symmetric])
qed
...
proof -
  ...
  also have "\<dots> = (i mod n + (n - i mod n)) mod n"
    by (simp add: pull_mods(8) [symmetric])
...
proof -
  ...
  also have "\<dots> = (i mod n + ((n - j mod n) + j) mod n) mod n"
    by (simp add: pull_mods(1) [symmetric])
...
proof (cases "0 \<le> sint x")
  ...
  with True show ?thesis by (simp add: is_signed_positive_def uint_sint bintrunc_mod2p)
...
next
  ...
  ultimately have "- sint x = - sint x mod 2 ^ LENGTH('a)" by simp
...
why3_open "bv/BV8.xml"
  constants
    ...
    ones=max_word
    bw_and=Bit_Operations.semiring_bit_operations_class.and
    bw_or=Bit_Operations.semiring_bit_operations_class.or
    bw_xor=Bit_Operations.semiring_bit_operations_class.xor
    bw_not=Bit_Operations.ring_bit_operations_class.not
(*
    bw_and=bitAND
    bw_or=bitOR
    bw_xor=bitXOR
...
why3_vc asr_zeros by (simp add: sshiftr_eq_funpow_sshiftr1)
...
why3_vc Nth_bv_is_nth
  by (metis bv_nth_def uint_ge_0 unat_eq_nat_uint)
(*by (simp add: bv_nth_def unat_def)*)

why3_vc Nth_bv_is_nth2
  using assms
  by (metis Nth_bv_is_nth to_uint_of_int)
...
why3_vc to_uint_add_bounded
  using assms
  by (simp add: uint_word_arith_bintrs bintrunc_mod2p)
...
why3_vc rotate_left_bv_is_rotate_left by auto (* by (simp add: unat_def) *)

why3_vc rotate_right_bv_is_rotate_right by auto (*  by (simp add: unat_def) *)

why3_vc lsr_bv_is_lsr by auto (* by (simp add: unat_def) *)

why3_vc to_uint_lsr sorry
(*  by (simp add: ediv_def shiftr_div_2n unat_def) *)

why3_vc asr_bv_is_asr by auto (* by (simp add: unat_def)*)

why3_vc lsl_bv_is_lsl by auto (*by (simp add: unat_def)*)

why3_vc to_uint_lsl sorry
...
why3_vc eq_sub_bv_def
  by (simp add: eq_sub_bv_defn mask_eq)
...
why3_vc Nth_bv_is_nth2
  using assms
  by (metis BV16.facts.Nth_bv_is_nth BV16.facts.to_uint_of_int)
(*  by (simp add: bv_nth_def unat_def to_uint_of_int uint_in_range_def) *)
...
why3_vc to_uint_ones sorry (* by (simp add: max_word_eq) *)
...
why3_vc Nth_bv_is_nth2
  using assms
  by (metis BV32.facts.Nth_bv_is_nth BV32.facts.to_uint_of_int)
(*  by (simp add: bv_nth_def unat_def to_uint_of_int uint_in_range_def) *)
...
why3_vc Nth_bv_is_nth2
  using assms BV64.facts.to_uint_of_int bv_nth_def by auto
(*  by (simp add: bv_nth_def unat_def to_uint_of_int uint_in_range_def) *)
...
why3_vc rotate_left_bv_is_rotate_left by simp  (* by (simp add: unat_def) *)

why3_vc rotate_right_bv_is_rotate_right by simp (* by (simp add: unat_def) *)

why3_vc lsr_bv_is_lsr by simp (* by (simp add: unat_def) *)

why3_vc to_uint_lsr sorry
(*  by (simp add: ediv_def shiftr_div_2n unat_def) *)

why3_vc asr_bv_is_asr by simp (*by (simp add: unat_def) *)

why3_vc lsl_bv_is_lsl by simp (* by (simp add: unat_def) *)
